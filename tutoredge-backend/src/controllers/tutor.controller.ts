...
export function generateTutorSlug(t: any): string {
  const rawName = t.fullName || t.name || "tutor";
  const name =
    rawName
      .toLowerCase()
      .trim()
  ...
  let cls = "";
  if (Array.isArray(t.classesTaught) && t.classesTaught.length > 0) {
    cls = t.classesTaught[0];
  } else if (typeof t.classesTaught === "string" && t.classesTaught) {
    cls = t.classesTaught.split(",")[0];
  ...
  const rawCity = t.location?.city || t.city || "";
  const city = rawCity
    .toLowerCase()
    .trim()
    .replace(/[^a-z0-9\s-]/g, "")
...
// 🔹 Get All Tutors (Public & Admin with Filtering & Pagination)
async getAllTutors(req: FastifyRequest, reply: FastifyReply) {
  try {
    const page = Number((req.query as any).page) || 1;
    const limit = Math.min(Number((req.query as any).limit) || 1000, 2000);
    ...
    if (query.subject) {
      filter.subjects = { $regex: new RegExp(query.subject, "i") };
    }
    if (query.city) {
    ...
    if (query.q) {
      ...
      filter.$or = [
        ...
        { subjects: qRegex },
        { headline: qRegex },
        { "location.city": qRegex },
        { "location.area": qRegex },
      ];
    }

    const tutors = await User.find(filter)
      .select("-password -location.pincode -location.coordinates")
      .sort({ rating: -1, profileViews: -1, createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .lean();
...
// 🔹 Get Tutor By ID or Slug (Public Profile)
async getTutorById(req: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    if (!tutor) {
      ...
      const targetSlug = id.toLowerCase().trim();

      // 1. Try exact computed slug match
      tutor = tutors.find((t: any) => generateTutorSlug(t) === targetSlug);

      // 2. Try prefix match (must start with nameSlug + "-")
      if (!tutor) {
        tutor = tutors.find((t: any) => {
          const nameSlug = (t.fullName || t.name || "")
            .toLowerCase()
            .trim()
            .replace(/[^a-z0-9\s-]/g, "")
            .replace(/\s+/g, "-");
          return nameSlug.length >= 3 && targetSlug.startsWith(nameSlug + "-");
        });
      }

      // 3. Try exact nameSlug match
      if (!tutor) {
        tutor = tutors.find((t: any) => {
          const nameSlug = (t.fullName || t.name || "")
            .toLowerCase()
            ...
            .replace(/[^a-z0-9\s-]/g, "")
            .replace(/\s+/g, "-");
          return nameSlug.length >= 3 && targetSlug === nameSlug;
        });
      }
    }
    ...
    const computedSlug = generateTutorSlug(tutor);
    ...
    console.error(err);
    return reply.status(400).send({ message: "Invalid tutor id or slug" });
  }
}
...
if (req.body.location && typeof req.body.location === "object") {
 ...
 allowedLocationFields.forEach((field) => {
  if (req.body.location[field] !== undefined) {
    ...
    if (field === "pincode") {
      updates.location.pincode = String(req.body.location.pincode);
    }
      ...
      updates.location[field] = req.body.location[field];
    }
  }
});
...
const docsComplete = !!(
  ...
  tutor.documents?.bankDetails
    ...
    const shouldBeComplete = completionPercent >= 80 && docsComplete;
...
// upload document 
async uploadTutorDocuments(req: any, reply: any) {
...
docs[part.fieldname] = fileUrl;
  // 🔥 YAHAN LAGAO (IMPORTANT)

        // 🔥 dynamic mapping (no if-else needed)
        // docs[part.fieldname] = fileUrl;
      }
    }
 console.log("FINAL DOCS 👉", docs);
    ...
    return reply.send({
      ...
      data: updatedUser?.documents || {} // 🔥 full docs return karo
    });
...
async uploadProfileImage(req: any, reply: any) {
  try {
    ...
    // ✅ FIX 1: timestamp ek baar
    const timestamp = Date.now();

    // ✅ FIX 2: unsafe filename clean karo
    const safeName = data.filename
      .replace(/\s+/g, "_")
      .replace(/[^\w.-]/g, "");

    // ✅ FIX 3: ONE filename everywhere
    const fileName = `${tutorId}-${timestamp}-${safeName}`;
...
async getDashboardSummary(req: FastifyRequest, reply: FastifyReply) {
  ...
  : {
      name: r.studentName,        // 🔥 manual lead support
      class_grade: r.classGrade, // 🔥 manual lead support
    },
  ...
  try {
    ...
    return reply.send({
      ...
      data: submissions.map((s) => ({
        ...
        studentName: s.submission?.student_name || (s.student_id as any)?.full_name,
...
/* =====================================
 ...
 // Class ke andar add karo
async uploadPhoto(req: any, reply: any) {
  try {
    const data = await req.file();
    ...
    return reply.send({ success: true, url: uploadResult.secure_url });
  } catch (err) {
    console.error("Tutor photo upload error:", err);
    return reply.status(500).send({ message: "Upload failed" });
  }
}
