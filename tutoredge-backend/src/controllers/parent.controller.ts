...
async getParentAssignmentById(
  ...
  try {
    ...
    const record = await StudentAssignment.findById(id)
      ...
      .populate("student_id", "full_name class_grade parent_id")
    ...
    const student = record.student_id as any;
    if (!student || String(student.parent_id) !== String(user.id)) {
      return reply.status(403).send({ error: "Forbidden - You don't have access to this assignment" });
    }
...
async uploadPhoto(req: any, reply: any) {
  try {
    ...
    // ✅ User model use karo, profileImage field already hai schema mein
    await User.findByIdAndUpdate((req as any).user.id, {
      profileImage: uploadResult.secure_url, // ✅ schema mein "profileImage" hai
    });
