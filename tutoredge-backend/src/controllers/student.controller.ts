...
async submitAssignment(
  ...
  try {
    ...
    const student = await Student.findById(studentAssignment.student_id).select("full_name parent_id");
    ...
    if (String(student.parent_id) !== String(user.id)) {
      return reply.status(403).send({
        error: "Forbidden - You don't have access to this student's assignment",
      });
    }
    ...
    // 🔥 STEP 3: UPDATE submission (NO CREATE)
    studentAssignment.submission = {
      files,
      submitted_at: new Date(),
      submitted_by: "parent",
