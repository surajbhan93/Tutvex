...
const [uploading, setUploading] = useState(false);
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setUploading(true);
      const res = await apiClient.post("/parent/upload-photo", formData, {
 ...
 const handleAddStudent = async () => {
  ...
  return (
    <ParentDashboardLayout>
                  <ParentTopBar
            parentName={form.fullName}
            studentName={students[0]?.full_name || "No student"}
...
<div className="flex items-center gap-4 mb-6">
  <div className="relative">
    ...
    <label className="absolute bottom-0 right-0 bg-blue-600 text-white 
                       ...
                       cursor-pointer text-xs hover:bg-blue-700">
      ✏️
      <input
        type="file"
        accept="image/*"
  ...
  <div>
    ...
    <p className="text-sm text-gray-500">
      {uploading ? "Uploading..." : "Click pencil to change photo"}
    </p>
  </div>
</div>
