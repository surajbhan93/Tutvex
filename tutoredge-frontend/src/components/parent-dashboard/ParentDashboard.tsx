...
export default function ParentDashboard() {
  ...
  // Ye state counts + payments ke liye hai.
 ...
 useEffect(() => {
  const loadDashboard = async () => {
    try {
      // 1️⃣ Profile (mandatory)
      const profileRes = await apiClient.get("auth/parent/profile");
      setParent(profileRes.data.parent);

      // 2️⃣ Students (already working)
      const studentsRes = await apiClient.get("/parent/students");
      const studentsList = studentsRes.data.students || [];
      setStudents(studentsList);
      setActiveStudent(studentsList[0] || null);

// ✅ TUTORS COUNT (NEW – REAL API)
      const tutorsRes = await apiClient.get("/auth/parent/my-tutors");
      const tutorsList = tutorsRes.data.data || [];

      // ✅ PAYMENTS SUMMARY (NEW – REAL API)
      const paymentRes = await apiClient.get(
        "/payments/parent/summary"
      );
      const paymentData = paymentRes.data.data;

      // 5️⃣ ASSIGNMENTS (NEW – REAL DATA)
      const assignmentRes = await apiClient.get("/parent/assignments");
      ...
      // 3️⃣ FRONTEND STATS (NO EXTRA API)
      // ✅ SET REAL STATS
      setStats({
        studentsCount: studentsList.length,
        tutorsCount: tutorsList.length,
        totalPaid: paymentData?.totalPaid || 0,
       pendingAssignments: pendingCount,
        upcomingSession: "",   // future API
      });
...
/>
          ...
          {[
  {
    ...
    value: stats.upcomingSession || "No session scheduled",
  },
  {
